...
write.matrix(dataFilt, 'dataFilt.csv', sep=",")
```
...
```{r}
write.matrix(colData(tcga_data)$shortLetterCode, 'y.csv')
```

```{r}
length(colData(tcga_data)$shortLetterCode)
```
...
```{r}
dataFilt <- read.csv('dataFilt.csv', sep=',')
head(dataFilt)
```
